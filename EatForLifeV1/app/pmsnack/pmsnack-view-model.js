...
function PmsnackViewModel(db) {
    ...
    viewModel.saveChanges = function () {
        ...
        if (pick1 != pick2 && pick2 != pick3 && pick3 != pick1) {
            db.all("SELECT idItem from item WHERE Description = ? OR Description = ? OR Description = ?", [pick1, pick2]).then(rows => {
                ...
                db.execSQL("UPDATE list SET idItem12 = ?, idItem13 = ? WHERE idList = ?", [item1, item2, listID]).then(id => {
