...
function AmsnackViewModel(db) {
    ...
    viewModel.itemList = new ObservableArray(4);
    ...
    viewModel.saveChanges = function () {
        ...
        var index4 = viewModel.get("selectedListPickerIndex3");
        ...
        if (pick1 != pick2 && pick2 != pick3 && pick3 != pick1 && pick4 != pick1 && pick4 != pick2 && pick4 != pick3) {
            db.all("SELECT idItem from item WHERE Description = ? OR Description = ? OR Description = ? OR Description = ?", [pick1, pick2, pick3,pick4]).then(rows => {
                ...
                db.execSQL("UPDATE list SET idItem4 = ?, idItem5 = ?, idItem6 = ?, idItem7 = ? WHERE idList = ?", [item1, item2, item3, item4, listID]).then(id => {
